...
public class aBall extends Thread {
		...
		while(true) {
			...
			if ((((vt/g)*(vy+vt)*(1-Math.exp(-g*t/vt))-vt*t+yi)<=2*bSize)&(vy1<0)) {
				...
				if (KEx <= 0.001*KEx0||KEy <= 0.001*KEy0) { 
					myBall.setLocation(xScr(x1),(yScr(floor)-xScr(2*bSize)));
					if(TEST) {
					System.out.print("Set the final location to x: " + xScr(x1) + " "+"y: "+(yScr(floor)-bSize)+"\n" );
	...
	public aBall(double xi, double yi, double v0, double theta, double bSize, Color bColor, double bLoss) {
		this.xi = xi;
		this.yi = yi;
		this.v0 = v0;
		this.theta = theta;
		...
		myBall = new GOval(xScr(xi),xScr(yi)+2*xScr(bSize),2*xScr(bSize),2*xScr(bSize));
